...
function checkParentChild(getVal){
	...
	if(nodeStatus == 'parent'){
		if($('#'+getVal).is(':checked')){
			//alert(document.querySelector('.'+getVal).id);
			$("#hiddenDoCreate"+getVal).attr("disabled", false);
			$("#hiddenDoUpdate"+getVal).attr("disabled", false);
			$('.'+getVal).attr('checked',true);
			var myList = document.getElementsByClassName(getVal);
			for(var a=0; a<myList.length; a++){
				$('.'+myList[a].id).attr('checked',true);
			}
		}else{
			...
			var myList = document.getElementsByClassName(getVal);
			for(var a=0; a<myList.length; a++){
				$('.'+myList[a].id).attr('checked',false);
			}
		}
