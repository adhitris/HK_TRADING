...
a(n).trigger("reloadGrid", [{current:true}]):!0===n.p.treeGrid?a(n).jqGrid("addChildNode",e[2],q,y):a(n).jqGrid("addRowData",e[2],y,b.addedrow),d[n.p.id].closeAfterAdd?(!0!==n.p.treeGrid&&a(n).jqGrid("setSelection",e[2]),a.jgrid.hideModal("#"+a.jgrid.jqID(w.themodal),{gb:"#gbox_"+a.jgrid.jqID(s),jqm:b.jqModal,onClose:d[n.p.id].onClose})):d[n.p.id].clearAfterAdd&&h("_empty",n,t)):(d[n.p.id].reloadAfterSubmit?(a(n).trigger("reloadGrid", [{current:true}]),d[n.p.id].closeAfterEdit||setTimeout(function(){a(n).jqGrid("setSelection",y[l])},
...
a("div.treeclick",l).removeClass(this.p.treeIcons.plus+" tree-plus").addClass(this.p.treeIcons.minus+" tree-minus"),this.p.treeANode=l.rowIndex,this.p.datatype=this.p.treedatatype,"nested"===this.p.treeGridModel?a(this).jqGrid("setGridParam",{postData:{nodeid:k,n_left:d[h],n_right:d[g],n_level:d[c]}}):a(this).jqGrid("setGridParam",{postData:{nodeid:k,parentid:d[b],n_level:d[c]}}),a(this).trigger("reloadGrid", [{current:true}]),d[f]=!0,"nested"===this.p.treeGridModel?a(this).jqGrid("setGridParam",{postData:{nodeid:"",
